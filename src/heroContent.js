...
const showDefaultWeather = () => {
  document.addEventListener("DOMContentLoaded", () => {
    async function displayWeather() {
      ...
      //   const getInfoOfWeather = showContentOfWeather(storeWeatherData);
      //   displayWeatherRetrieved.append(showContentOfWeather(storeWeatherData));
      //   return getInfoOfWeather;
...
const showContentOfWeather = (fetchedWeather) => {
  weatherContent.forEach((item) => {
    const containerDivForAll = document.createElement("div");
    containerDivForAll.textContent = "";
    const options = {
      weekday: "long",
      year: "numeric",
      month: "long",
